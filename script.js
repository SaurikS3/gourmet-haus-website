...
class ContactFormHandler {
    ...
    async handleSubmit(e) {
        ...
        try {
            ...
            this.showMessage(
                'Sorry, there was an error sending your message. Please try again or contact us directly at (703) 867-5112.',
                'error'
            );
        } finally {
    ...
    setLoading(isLoading) {
        if (this.submitBtn) {
            if (isLoading) {
                ...
                this.submitBtn.querySelector('.btn-text').textContent = 'SEND MESSAGE';
            }
        }
    }
    ...
    showMessage(text, type) {
        if (this.messageDiv) {
            ...
            this.messageDiv.className = `form-message ${type} visible`;
            
            // Auto-hide success messages after 5 seconds
            if (type === 'success') {
                setTimeout(() => {
                    this.messageDiv.classList.remove('visible');
                }, 5000);
            }
        }
    }
...
document.addEventListener('DOMContentLoaded', () => {
    // Add field error animation style
    const style = document.createElement('style');
    style.textContent = `
        @keyframes fadeIn {
            from {
